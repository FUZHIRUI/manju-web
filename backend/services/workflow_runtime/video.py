...
async def run_video_prepare_prompts(
    ...
    """Step 1: prepare + phase1_video_prompts — 项目名提取、章节发现、fenjing_prompts 下载、视频提示词生成"""
    if not project_name:
        if "storyboard_assets" in output_dir.parts:
            project_idx = output_dir.parts.index("storyboard_assets")
            if project_idx > 0:
    ...
    for chapter_file in chapters:
        chapter_name_local = chapter_file.stem
        chapter_dir = storyboards_dir / chapter_name_local
        fenjing_prompts_path = chapter_dir / "fenjing_prompts.jsonl"
        if not fenjing_prompts_path.exists():
            key = f"{assets_prefix.rstrip('/')}/storyboards/{chapter_name_local}/fenjing_prompts.jsonl"
            _video_download_file_from_tos(bucket, key, fenjing_prompts_path)
        if not fenjing_prompts_path.exists():
            prefix_key = f"{assets_prefix.rstrip('/')}/storyboards/{chapter_name_local}/"
            keys = list_tos_keys(bucket, prefix_key)
            fallback_key = None
            for k in keys:
                if isinstance(k, str) and k.endswith("fenjing_prompts.jsonl"):
                    fallback_key = k
    ...
    async def _build_prompts(ch_name: str, fp_path: Path, ch_dir: Path) -> Path:
        vp = ch_dir / "shipin_prompts.jsonl"
        if vp.exists():
            return vp
    ...
    prompt_jobs = []
    for entry in chapter_entries:
        task = asyncio.create_task(_build_prompts(entry["chapter_name"], entry["fenjing_prompts_path"], entry["chapter_dir"]))
        prompt_jobs.append(task)
        if interval > 0:
            await asyncio.sleep(interval)
...
async def run_video_generate_only(
    ...
    """Step 2: phase2_video_generation — 扫描已有 shipin_prompts.jsonl 章节，生成视频（下载到本地，不上传 TOS）"""
    if not project_name:
        if "storyboard_assets" in output_dir.parts:
            project_idx = output_dir.parts.index("storyboard_assets")
            if project_idx > 0:
    ...
    # 扫描已有 shipin_prompts.jsonl 的章节
    chapter_entries = []
    if storyboards_dir.exists():
        for chapter_dir in sorted(storyboards_dir.iterdir()):
            if not chapter_dir.is_dir():
                continue
            vp = chapter_dir / "shipin_prompts.jsonl"
            fp = chapter_dir / "fenjing_prompts.jsonl"
            if vp.exists() and fp.exists():
                chapter_entries.append({
                    "chapter_name": chapter_dir.name,
                    "fenjing_prompts_path": fp,
                    "video_prompts_path": vp,
                    "chapter_dir": chapter_dir,
                })
    ...
    for entry in chapter_entries:
        chapter_name = entry["chapter_name"]
        fenjing_prompts_path = entry["fenjing_prompts_path"]
        video_prompts_path = entry["video_prompts_path"]
        video_dir = output_dir / "video" / chapter_name
        ensure_dir(video_dir)

        prompts = read_jsonl(str(video_prompts_path))
        fenjing_prompts = read_jsonl(str(fenjing_prompts_path))
        fenjing_duration_map = {str(item.get("fenjing_id", "")): item.get("duration", 5.0) for item in fenjing_prompts}

        tasks = await submit_video_tasks_batch(
            prompts, fenjing_duration_map, video_dir,
            qps=qps, chapter_name=chapter_name, project_name=project_name,
            skip_upload=True
        )
...
async def run_video_upload_only(
    ...
    """Step 3: fenjing_video_upload — 扫描本地已下载 MP4 并上传 TOS"""
    if not project_name:
        if "storyboard_assets" in output_dir.parts:
            project_idx = output_dir.parts.index("storyboard_assets")
            if project_idx > 0:
    ...
    for chapter_dir in sorted(video_base.iterdir()):
        if not chapter_dir.is_dir():
            continue
        chapter_name = chapter_dir.name
        mp4_files = sorted(chapter_dir.glob("fenjing_*_video.mp4"))
        for mp4 in mp4_files:
            tos_key = f"{tos_video_prefix}/{chapter_name}/{mp4.name}" if chapter_name else f"{tos_video_prefix}/{mp4.name}"
            emit_event(
            ...
            if uploaded_url:
                success_count += 1
                fenjing_id = mp4.stem.replace("fenjing_", "").replace("_video", "")
                emit_event(
